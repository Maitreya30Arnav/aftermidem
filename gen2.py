...
def autocorr(x, max_lag=50):
    ...
    for lag in range(1, max_lag):
        if lag >= len(x):
            break
        val = np.corrcoef(x[:-lag], x[lag:])[0,1]
        acf.append(0 if np.isnan(val) else val)
    return np.array(acf)
...
def generate_synthetic_and_metrics(df, start_row, end_row):
    ...
    phi = np.clip(acf_lag1 if not np.isnan(acf_lag1) else 0.9, 0.7, 0.98)
    ...
    rms_error = abs(rms_real - rms_syn) / (rms_real + 1e-8) * 100

    dt = np.median(np.diff(t_real)) if len(t_real) > 1 else 1
    fs = 1000 / dt

    f_real, psd_real = welch(x_real, fs=fs, nperseg=min(1024, len(x_real)))
    f_syn,  psd_syn  = welch(syn, fs=fs, nperseg=min(1024, len(syn)))

    psd_peak_error = abs(np.max(psd_real) - np.max(psd_syn)) / (np.max(psd_real)+1e-8) * 100

    acf_real = autocorr(x_real)
    acf_syn  = autocorr(syn)

    if len(acf_real) > 2 and len(acf_syn) > 2:
        min_len = min(len(acf_real), len(acf_syn))
        acf_corr, _ = pearsonr(acf_real[:min_len], acf_syn[:min_len])
    else:
        acf_corr = 0

    kurt_real = kurtosis(x_real)
    kurt_syn  = kurtosis(syn)
    kurt_error = abs(kurt_real - kurt_syn) / (abs(kurt_real)+1e-8) * 100
